"""
Fine-tuned RoBERTa Classifier for Aspect-Based Sentiment Analysis (ABSA)

Architecture:
  Model 1 — Aspect Classifier:
      Input : sentence
      Encode: <s> sentence </s>  →  RoBERTa  →  [CLS] representation
      Head  : Linear(768 → 5)  +  BCEWithLogitsLoss  (multi-label)

  Model 2 — Sentiment Classifier:
      Input : sentence + aspect name pair
      Encode: <s> sentence </s></s> aspect_name </s>  →  RoBERTa  →  [CLS]
      Head  : Linear(768 → 4)  +  CrossEntropyLoss

Note: RoBERTa does NOT use token_type_ids.

Hyperparameters follow the RoBERTa paper (Liu et al. 2019):
  - LR            : 2e-5
  - Warmup ratio  : 0.06 (6%)
  - Weight decay  : 0.01
  - Batch size    : 16
...
# ── Config ────────────────────────────────────────────────────────────────────
# RoBERTa paper (Liu et al. 2019) fine-tuning hyperparameters
ROBERTA_MODEL  = 'roberta-base'
MAX_LEN        = 128
EPOCHS_ASPECT  = 10
EPOCHS_SENT    = 10
BATCH_SIZE     = 16
LR             = 2e-5        # paper: {1e-5, 2e-5, 3e-5}
WARMUP_RATIO   = 0.06        # paper: 6% warmup
...
OUT_FILE     = os.path.join(DATA_DIR, 'roberta_test_pred.csv')
DEV_OUT_FILE = os.path.join(DATA_DIR, 'roberta_dev_pred.csv')
...
class AspectDataset(Dataset):
    ...
    RoBERTa does not use token_type_ids — excluded here.
    """
...
class SentimentDataset(Dataset):
    ...
    RoBERTa sentence-pair: <s> sentence </s></s> aspect </s>
    """
    def __init__(self, df, tokenizer):
        enc = tokenizer(
            df['text'].tolist(),
...
# ── RoBERTa Models ────────────────────────────────────────────────────────────
class RoBERTaAspect(nn.Module):
    """Multi-label aspect classifier using <s> (CLS) token from RoBERTa."""
    def __init__(self, roberta, n_aspects, dropout):
        super().__init__()
        self.roberta = roberta
        self.dropout = nn.Dropout(dropout)
        self.fc      = nn.Linear(roberta.config.hidden_size, n_aspects)
...
class RoBERTaSentiment(nn.Module):
    ...
    def __init__(self, roberta, n_sentiments, dropout):
        super().__init__()
        self.roberta = roberta
        self.dropout = nn.Dropout(dropout)
        self.fc      = nn.Linear(roberta.config.hidden_size, n_sentiments)
...
def main():
    ...
    # ── Aspect model ─────────────────────────────────────────────────────────
    print("\n=== Training RoBERTa Aspect Classifier ===")
    ...
    asp_roberta = AutoModel.from_pretrained(ROBERTA_MODEL)
    asp_model   = RoBERTaAspect(asp_roberta, len(ASPECTS), DROPOUT).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS_ASPECT + 1):
        ...
        eta = sum(asp_epoch_times) / len(asp_epoch_times) * (EPOCHS_ASPECT - epoch)
    ...
    sent_roberta = AutoModel.from_pretrained(ROBERTA_MODEL)
    sent_model   = RoBERTaSentiment(sent_roberta, len(SENTIMENTS), DROPOUT).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS_SENT + 1):
        ...
        eta = sum(sent_epoch_times) / len(sent_epoch_times) * (EPOCHS_SENT - epoch)
