...
@torch.no_grad()
def compute_dev_loss(model, asp_loader, sent_loader, asp_crit, sent_crit) -> float:
    """
    Compute the same joint loss used during training, but on the dev set.
    Returns the average combined loss (aspect + sentiment).
    """
    model.eval()
    total_loss = 0.0
    n_steps    = max(len(asp_loader), len(sent_loader))
    ...
    for _ in range(n_steps):
        ...
        asp_labels  = asp_batch.pop('labels')
        sent_labels = sent_batch.pop('labels')
        asp_loss  = asp_crit(model.forward_aspect(**asp_batch), asp_labels)
        sent_loss = sent_crit(model.forward_sentiment(**sent_batch), sent_labels)
        total_loss += (ASP_LOSS_WEIGHT * asp_loss + SENT_LOSS_WEIGHT * sent_loss).item()

    return total_loss / n_steps


def plot_losses(train_losses, dev_losses, out_path):
    """Save a train/dev loss curve to `out_path`."""
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training vs Dev Loss per Epoch')
    plt.legend()
...
def main():
    ...
    print(f"  {'Epoch':>5}  {'Train Loss':>10}  {'Dev Loss':>10}  {'Dev F1':>8}  {'Time':>7}  ETA")
    print("  " + "-" * 60)
    ...
    for epoch in range(1, EPOCHS + 1):
        t0         = time.time()
        train_loss = train_epoch_joint(model, asp_dl_train, sent_dl_train,
                                       optimizer, scheduler, asp_crit, sent_crit)
        dev_loss   = compute_dev_loss(model, asp_dl_dev, sent_dl_dev,
                                      asp_crit, sent_crit)
        train_losses.append(train_loss)
        dev_losses.append(dev_loss)

        # Quick dev F1 evaluation
        dev_unique  = dev_df.drop_duplicates('id')
        ...
        print(f"  {epoch:5d}  {train_loss:10.4f}  {dev_loss:10.4f}  {f1:8.4f}  "
              f"{elapsed:6.0f}s  {eta/60:.1f}min")
        if f1 > best_f1:
            best_f1    = f1
