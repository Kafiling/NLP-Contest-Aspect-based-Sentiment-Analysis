"""
Fine-tuned ModernBERT Classifier for Aspect-Based Sentiment Analysis (ABSA)

Architecture:
  Model 1 — Aspect Classifier:
      Input : sentence
      Encode: [CLS] sentence [SEP]  →  ModernBERT  →  [CLS] representation
      Head  : Linear(768 → 5)  +  BCEWithLogitsLoss  (multi-label)

  Model 2 — Sentiment Classifier:
      Input : sentence + aspect name pair
      Encode: [CLS] sentence [SEP] aspect_name [SEP]  →  ModernBERT  →  [CLS]
      Head  : Linear(768 → 4)  +  CrossEntropyLoss

Note: ModernBERT uses RoPE positional embeddings and does NOT use token_type_ids.

Hyperparameters follow the ModernBERT paper (arxiv:2412.13663):
  - LR            : 5e-5 (higher than classic BERT, as recommended for ModernBERT)
  - Warmup ratio  : 0.06 (6%)
  - Weight decay  : 0.01
  - Batch size    : 16
...
# ── Config ─────────────────────────────────────────────────────────────────
# ModernBERT paper fine-tuning hyperparameters (arxiv:2412.13663)
MODERNBERT_MODEL = 'answerdotai/ModernBERT-base'
MAX_LEN          = 128          # sentences are short; 128 is sufficient
EPOCHS_ASPECT    = 10
EPOCHS_SENT      = 10
BATCH_SIZE       = 16
LR               = 5e-5         # paper: higher LR than classic BERT
WARMUP_RATIO     = 0.06         # paper: 6% warmup steps
WEIGHT_DECAY     = 0.01         # paper: 1e-2
DROPOUT          = 0.1
ASPECT_THRESH    = 0.4
DEV_SPLIT        = 0.15         # same split strategy as other models
...
OUT_FILE     = os.path.join(DATA_DIR, 'modernbert_test_pred.csv')
DEV_OUT_FILE = os.path.join(DATA_DIR, 'modernbert_dev_pred.csv')
...
class AspectDataset(Dataset):
    ...
    ModernBERT does not use token_type_ids — excluded here.
    """
...
# ── ModernBERT Models ─────────────────────────────────────────────────────────
class ModernBERTAspect(nn.Module):
    """Multi-label aspect classifier using [CLS] from ModernBERT."""
    def __init__(self, encoder, n_aspects, dropout):
        super().__init__()
        self.encoder = encoder
        self.dropout = nn.Dropout(dropout)
        self.fc      = nn.Linear(encoder.config.hidden_size, n_aspects)
...
class ModernBERTSentiment(nn.Module):
    """Sentiment classifier conditioned on aspect via sentence-pair encoding."""
    def __init__(self, encoder, n_sentiments, dropout):
        super().__init__()
        self.encoder = encoder
        self.dropout = nn.Dropout(dropout)
        self.fc      = nn.Linear(encoder.config.hidden_size, n_sentiments)
...
def main():
    ...
    # ---- Train / dev split by unique ID (same method as other models) ----
    ...
    # ---- Tokenizer (AutoTokenizer resolves to ModernBERT's tokenizer) ----
    print(f"\nLoading tokenizer: {MODERNBERT_MODEL}")
    tokenizer = AutoTokenizer.from_pretrained(MODERNBERT_MODEL)

    # ── Aspect model ─────────────────────────────────────────────────────────
    print("\n=== Training ModernBERT Aspect Classifier ===")
    ...
    asp_encoder = AutoModel.from_pretrained(MODERNBERT_MODEL)
    asp_model   = ModernBERTAspect(asp_encoder, len(ASPECTS), DROPOUT).to(DEVICE)
    asp_optim   = torch.optim.AdamW(asp_model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    ...
    # ── Sentiment model ───────────────────────────────────────────────────────
    print("\n=== Training ModernBERT Sentiment Classifier ===")
    ...
    sent_encoder = AutoModel.from_pretrained(MODERNBERT_MODEL)
    sent_model   = ModernBERTSentiment(sent_encoder, len(SENTIMENTS), DROPOUT).to(DEVICE)
    sent_optim   = torch.optim.AdamW(sent_model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
